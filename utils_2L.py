...
def stationary_max(grid, out, u = None, K = None, precond_type = 'ML_ORAS'):
    ...
    # out_lmax = spml.SparseCSRTensor(copy.deepcopy(u))
    ...
    for k in range(K):
        # out_lmax = eprop @ out_lmax
        out_lmax = stationary_cycle(tsA, M, R0, out_lmax)
        l2 = torch.norm(out_lmax, p='fro', dim = 0)
...
def test_stationary(grid, output, precond_type, u, K, M=None):
    ...
    if M is None:
        M, _ = preconditioner(grid, output, train = False, precond_type = precond_type, u = u)
