...
def generate_data(data_args, show_fig = False):
    ...
    path = data_args.directory+'Grids-'+data_args.PDE+'-'+data_args.BC
    ...
    if data_args.structured:
        ...
        for i in range(data_args.num_data):
            ...
            while num_dom < 2:
                ...
                lcmin = np.random.uniform(0.15, 0.21)

                lcmax = np.random.uniform(0.21, 0.25)
            ...
            if show_fig:
                grid.plot_agg(size = 3, labeling = False, w = 0.1,shade = 0.01)
                plt.show()
