...
def test_stats(grid, dict_stats, list_test):
    ...
    u = torch.rand(grid.A.shape[0],100).double()
...
if __name__ =='__main__':
    ...
    #     lcmin = 0.04#np.random.uniform(0.08, 0.09)
    #     lcmax = 0.045#np.random.uniform(0.14, 0.15)
    ...
    # grid = torch.load('Data/TG5k/grid10.pth')
    ...
    model = HGNN(lvl=2, dim_embed=128, num_layers=6, K= 6, ratio=0.025, lr=1e-4)
    # model = mloras_net(dim = 128, K = 2, num_res = 8, num_convs = 4, lr = 1e-3, res = True, tf=False)
    
    directory  = 'Models/Model-TG-new/model_epoch_best.pth'
    ...
    for name in list_test:
        ...
        if num_lvl == 2:
            ...
            if name == 'ML_ORAS':
                dict_precs[name] = [Mprec.to_dense().detach().numpy(), out[1]]
            else:
                dict_precs[name] = [Mprec.to_dense().detach().numpy(), None]
